<template>
  <div class="gpu-skip-batch">
    <div class="panel">
      <div class="title">🚀 GPU 批量训练残差网络 · 圆环分类（mini-batch SGD · 等宽跳接）</div>
      <div class="fields">
        <label>隐藏层宽度
        ...
        <label>跳接跨度 skip
          <input type="number" v-model.number="cfg.skip" min="1" max="16" />
        </label>
        <label>样本数
    ...
    <p class="hint">💡 跳接只在等宽的隐藏层之间，最后一块不足整跨度时自动收缩为 min(span, 剩余层数)。batch &gt; 1 时一次 draw 处理一批样本。</p>
  </div>
</template>

<script setup lang="ts">
import { ref, onUnmounted } from 'vue'
import SkipBatchWorkerCtor from './gpuSkipBatchWorker.ts?worker'

/* 训练循环在 Web Worker（OffscreenCanvas + WebGL2）中执行，主线程只负责：
   ① 发送训练配置；② 收集训练日志；③ 绘制热力图与耗时。 */

const cfg = ref({ width: 8, depth: 5, skip: 2, samples: 800, batch: 32, lr: 0.3, epochs: 300, r1: 0.5, r2: 1.5 });
...
function run() {
    ...
    const w = new SkipBatchWorkerCtor();
    worker = w;
