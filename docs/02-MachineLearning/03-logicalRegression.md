...
其实我在了解到交叉熵函数之后，也曾经想构造一个损失函数，让模型在预测正确的时候，损失接近0，预测错误的时候损失能暴增，于是我大笔一挥写下了这个函数：
$\operatorname{Loss}=-(y(1-\frac{1}{a})+(1-y)(1-\frac{1}{1-a}))$
但是这个函数对激活函数求导有：
$\frac{\partial \operatorname{Loss}}{\partial σ}=-\frac{y}{a^2}+\frac{1-y}{(1-a)^2}$
这个导数和 $\frac{\partial σ}{\partial z}$ 相乘会非常复杂，除非我们也给配套的换一个激活函数。
由此可见可见：**在不同的任务中，选择合适的激活函数和损失函数，可以让我们的的训练如虎添翼**